"""Compare production ARM64 ty binaries on independent, pinned projects."""
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--platform", choices=("darwin", "linux"), default=sys.platform)
    parser.add_argument("--pairs", type=int, default=8)
    parser.add_argument("--warmups", type=int, default=1)
    parser.add_argument("--lsp-sessions", type=int, default=6)
    parser.add_argument("--timeout", type=float, default=90)
    args = parser.parse_args()
    if args.pairs < 2 or args.pairs % 2 or args.lsp_sessions < 1 or args.warmups < 1:
        parser.error("an even number of pairs and one warmup/LSP session are required")
    return args
...
def extract_binary(source: Path) -> Path:
    source = source.resolve(strict=True)
    if source.is_file() and source.name == "ty":
        return source
    executables = list(source.rglob("ty")) if source.is_dir() else []
    executables = [candidate for candidate in executables if candidate.is_file()]
    if len(executables) == 1:
        return executables[0].resolve(strict=True)
    raise RuntimeError(
        f"expected one extracted ty executable in {source}: {executables}"
    )


def process_usage(previous: resource.struct_rusage) -> dict[str, int]:
    current = resource.getrusage(resource.RUSAGE_CHILDREN)
    user_ns = round((current.ru_utime - previous.ru_utime) * 1_000_000_000)
    kernel_ns = round((current.ru_stime - previous.ru_stime) * 1_000_000_000)
    return {
...
def cli_once(
    ...
    *,
    ...
    result = process_usage(previous_usage)
...
def ordered_labels(round_number: int) -> tuple[str, ...]:
    ...
    return permutations[round_number % len(permutations)]
    ...
    try:
        initialized = client.request(
            "initialize",
            {
                ...
                "clientInfo": {"name": "ty-arm-pgo-heldout", "version": "1"},
        ...
        client.shutdown()
        usage = process_usage(previous_usage)
    finally:
...
def aggregate(report: dict[str, Any], category: str) -> dict[str, Any]:
    ...
    if not workloads:
        return {}
    metrics = ("wall_ns", "cpu_ns") if category == "cli" else ("edit", "initial", "cpu")
...
def markdown_report(report: dict[str, Any]) -> str:
    operating_system = "macOS" if report.get("python_platform") == "darwin" else "Linux"
    lines = [
        f"# {operating_system} ARM64 ty PGO benchmark",
        "",
        "Real production release executables, pinned held-out projects, "
        f"and a shared {operating_system} Python 3.12 environment.",
        "",
        "## CLI",
        "",
        "| Project | No PGO CPU | PGO CPU | CPU reduction | Wall reduction |",
        "| --- | ---: | ---: | ---: | ---: |",
    ]
    for workload in report.get("cli", []):
        ...
        lines.append(
            ...
            f"{summary['pgo']['cpu_ns_reduction_percent_vs_baseline']:+.2f}% | "
            f"{summary['pgo']['wall_ns_reduction_percent_vs_baseline']:+.2f}% |"
        )
    if cli := aggregate(report, "cli"):
        lines.append(
            "| **Geometric mean** | — | — | "
            f"**{cli['pgo']['cpu_ns_reduction_percent_vs_baseline']:+.2f}%** | "
            f"**{cli['pgo']['wall_ns_reduction_percent_vs_baseline']:+.2f}%** |"
        )
    lines.extend(
        [
            "",
            "## Language server",
            "",
            "| Project | No PGO edit | PGO edit | Edit latency reduction |",
            "| --- | ---: | ---: | ---: |",
        ]
    )
    for workload in report.get("lsp", []):
        ...
        lines.append(
            ...
            f"{summary['pgo']['edit_median_ms']:.2f} ms | "
            f"{summary['pgo']['edit_reduction_percent_vs_baseline']:+.2f}% |"
        )
    if lsp := aggregate(report, "lsp"):
        lines.append(
            "| **Geometric mean** | — | — | "
            f"**{lsp['pgo']['edit_reduction_percent_vs_baseline']:+.2f}%** |"
        )
    ...
    lines.extend(
        [
            ...
            "Execution order alternates across paired rounds.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    args = arguments()
    if args.platform != sys.platform or sys.platform not in {"darwin", "linux"}:
        raise RuntimeError("this benchmark requires a native macOS or Linux runner")
    architecture = platform.machine().lower()
    if architecture not in {"arm64", "aarch64"}:
        raise RuntimeError(f"this benchmark requires native ARM64: {architecture}")
    ...
    try:
        if hasattr(os, "sched_getaffinity") and hasattr(os, "sched_setaffinity"):
            ...
            if not affinity:
                raise RuntimeError("the runner has no available processors")
            os.sched_setaffinity(0, affinity)
            parallelism = len(affinity)
            report["cpu_affinity"] = sorted(affinity)
        else:
            parallelism = os.cpu_count() or 1
        report["parallelism"] = parallelism
        binaries: dict[str, Path] = {}
        for label in LABELS:
            binary = extract_binary(getattr(args, label))
        ...
        for project in PROJECTS:
            ...
            if (
                ...
                outputs = {
                    label: cli_once(binary, project, checkout, python, 1, capture=True)
                    for label, binary in binaries.items()
                }
                if (
                    len(
            ...
            for index in range(args.warmups):
                for label in ordered_labels(index):
                    cli_once(binaries[label], project, checkout, python, parallelism)
            ...
            for index in range(args.pairs):
                ...
                for label in order:
                    observation[label] = cli_once(
                        binaries[label], project, checkout, python, parallelism
                    )
            ...
            checkpoint(report, args.json_output, args.markdown_output)
            optimized = workload["summary"]["pgo"]
            print(
                f"{project.name}: PGO CPU vs no PGO "
                f"{optimized['cpu_ns_reduction_percent_vs_baseline']:+.2f}%",
                flush=True,
            )
