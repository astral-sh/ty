...
=== "macOS and Linux"
    ...
    ```console
    $ curl -LsSf https://astral.sh/ty/0.0.13/install.sh | sh
    ```
...
=== "Windows"
    ...
    ```pwsh-session
    PS> powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/ty/0.0.13/install.ps1 | iex"
    ```
...
- `ghcr.io/astral-sh/ty:{major}.{minor}.{patch}`, e.g., `ghcr.io/astral-sh/ty:0.0.13`
